...
void ClassFileParser::verify_field_initval(const ConstantPool* const cp,
  ...
  guarantee_property(
    ...
    (is_lazyvalue
     ? "Bad lazy value index %u in LazyValue attribute in class file %s"
  ...
  if (is_lazyvalue) {
    ...
    if (required_field_type != NULL) {
      guarantee_property(field_type == required_field_type,
                         "Inconsistent type for LazyValue attribute in class file %s",
                         CHECK);
      return;
    }
    // Check for other CP types below.  It is not super-useful to make
    // CONSTANT_Integer etc. lazy as well as normal initial values,
...
void ClassFileParser::parse_field_attributes(const ClassFileStream* const cfs,
  ...
  assert(lazyvalue_index_addr != NULL, "invariant");
  ...
  while (attributes_count--) {
    ...
    } else if (is_static && attribute_name == vmSymbols::tag_lazy_value()) {
      // ignore if non-static
      if (lazyvalue_index != 0) {
        classfile_parse_error("Duplicate LazyValue attribute in class file %s", THREAD);
        return;
      }
      check_property(
        attribute_length == 2,
        "Invalid LazyValue field attribute length %u in class file %s",
        attribute_length, CHECK);

      lazyvalue_index = cfs->get_u2(CHECK);
      if (_need_verify) {
        verify_field_initval(cp, true, lazyvalue_index, signature_index, CHECK);
      }
  ...
  if (lazyvalue_index != 0 && constantvalue_index != 0) {
    classfile_parse_error("Clashing LazyValue and ConstantValue attributes in class file %s", THREAD);
    return;
  }

  *constantvalue_index_addr = constantvalue_index;
  *lazyvalue_index_addr = lazyvalue_index;
  ...
  *java_fields_count_ptr = length;
  ...
  for (int n = 0; n < length; n++) {
    ...
    int initval_index = (constantvalue_index != 0 ? constantvalue_index :
                         lazyvalue_index != 0 ? lazyvalue_index :
                         0);
    if (lazyvalue_index != 0) {
      access_flags.set_is_lazy_value();
    }
