...
void java_lang_Class::initialize_lazy_static_field(Klass* klass,
                                                   fieldDescriptor* fd,
                                                   TRAPS) {
  assert(klass == fd->field_holder(), "must be");
  assert(fd->is_static(), "must be");
  assert(fd->is_lazy_value(), "must be");

  // Initializing a lazy field may only happen after its declaring
  // class has been initialized.  It is harmless to make this request
  // redundantly.
  klass->initialize(CHECK);

  int initval = fd->lazy_value_index();
